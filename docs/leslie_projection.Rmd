...
# The line below... 9 x 4 matrix
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below...filling row
proj_year[1, ] <- fish_year0

# The for loop below... dot product not pipe operator
...
proj_df <- data.frame(time_yr, proj_year) %>% 
  #pivot longer essentially makes stacked plots, 0E:0A, 1E:1A, . . .
  #cols excludes the time year
  #change col names to and move to values
...
ggplot(data = demo_longer) +
  geom_line(aes(x = year, y = species))
```
